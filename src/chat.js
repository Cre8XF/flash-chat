...
import { avatarGradient, escapeHtml, formatTime, formatTTL } from './helpers.js';
...
export async function openChat(chatId, partnerId) {
  ...
  messagesUnsub = onSnapshot(msgQ, async (snap) => {
    ...
    const messages = snap.docs
      ...
      .filter(m => !m.expiresAt || m.expiresAt.toMillis() > now); // hide expired client-side
...
function renderMessages(messages, now) {
  ...
  container.innerHTML = messages.map(m => {
    ...
    // Read receipt — shown under sent messages only
    ...
    const ttlHtml = ttlMs != null
      ? `<span class="ttl-label">⏱ ${formatTTL(ttlMs)}</span>`
      : '';

    return `<div class="message-row ${isSent ? 'sent' : 'received'}">
...
// Upload to Cloudflare R2 via the pre-signed URL worker.
// Falls back to Firebase Storage when VITE_R2_WORKER_URL is not set.
